...
// Comparador único y robusto para ordenar alumnos en toda la app.
// - Nuevo ingreso siempre al final (sin importar sexo/orden alfabético).
// - Sexo normalizado (evita que una "v" minúscula rompa el agrupamiento).
// - localeCompare con sensitivity:'base' para que mayúsculas/acentos no den
//   resultados distintos entre navegadores/celulares (causa real del "desacomodo").
// - Desempate final por DNI para garantizar el MISMO orden siempre, sin importar el motor JS.
const compararAlumnos = (a, b) => {
  const nuevoA = !!a.nuevoIngreso, nuevoB = !!b.nuevoIngreso;
  if (nuevoA !== nuevoB) return nuevoA ? 1 : -1;
  const sexoA = (a.sexo || 'V').trim().toUpperCase() === 'M' ? 'M' : 'V';
  const sexoB = (b.sexo || 'V').trim().toUpperCase() === 'M' ? 'M' : 'V';
  if (sexoA !== sexoB) return sexoA === 'V' ? -1 : 1;
  const nombreA = (a.nombre || '').trim();
  const nombreB = (b.nombre || '').trim();
  const cmp = nombreA.localeCompare(nombreB, 'es', { sensitivity: 'base' });
  if (cmp !== 0) return cmp;
...
async function generarPDFUnificado({ usuario, alumnosGlobales, db, todosUsuarios, includeTalleres = false }) {
  ...
  try {
    ...
    for (const grado of gradosDocente) {
      ...
      const alumnosOrdenados = [...alumnosDelGrado].sort(compararAlumnos);
      ...
      // Nota solo para 1°-3°: Lengua y Literatura sigue siendo una sola materia en el sistema,
      // pero se recomienda desdoblar sus criterios de evaluación en oral y escrita.
      let finalYCurr = pdfDoc.lastAutoTable.finalY;
      if (primerCiclo) {
        pdfDoc.setFont('helvetica', 'italic'); pdfDoc.setFontSize(8); pdfDoc.setTextColor(90, 90, 90);
        pdfDoc.text('* Se recomienda "Lengua Oral" y "Lengua Escrita" como criterios de evaluación dentro de Lengua y Literatura.', PDF_MARGIN, finalYCurr + 6);
        finalYCurr += 6;
      }
      agregarFirma(finalYCurr + 10, grado);

      // ── Página 2: Áreas Especiales ──
      // Orden pedido: distinto para 1°-3° que para 4°-7°. Esto SOLO reordena cómo se
      // imprimen las columnas en este PDF — no cambia el orden en ningún otro lugar del sistema.
      const ordenEspeciales4a7 = ['Educación Artística: Música', 'Educación Artística: Plástica', 'Educación Física', 'Tecnología', 'Lengua Extranjera: Inglés', 'Lengua Extranjera: Portugués', 'Laboratorio', 'Informática'];
      const ordenEspeciales1a3 = ['Lengua Extranjera: Inglés', 'Tecnología', 'Educación Física', 'Educación Artística: Música', 'Educación Artística: Plástica'];
      const ordenEsp = primerCiclo ? ordenEspeciales1a3 : ordenEspeciales4a7;
      const especiales = [...areas.especiales].sort((a, b) => {
        const ia = ordenEsp.indexOf(a.nombre), ib = ordenEsp.indexOf(b.nombre);
      ...
      // ── Promedios Generales — suma/promedio de Curriculares (sin Convivencia) + Especiales ──
      ...
      // numérico interno y se muestra en formato conceptual.
      {
        ...
        pdfDoc.addPage();
        encabezado(`Promedios Generales — ${bimActivo} Bimestre`, grado);
        pdfDoc.setFont('helvetica', 'normal'); pdfDoc.setFontSize(9); pdfDoc.setTextColor(40, 40, 40);
        let y = 40;
        const lineHeight = 5.5;
        lineasProm.forEach(linea => {
          if (y > pageH - 25) { pdfDoc.addPage(); encabezado(`Promedios Generales — ${bimActivo} Bimestre (cont.)`, grado); y = 40; }
          pdfDoc.text(linea, PDF_MARGIN, y);
          y += lineHeight;
        });
        agregarFirma(y + 8, grado);
      }
...
export default function SistemaCalificaciones() {
  ...
  const [alumnoForm, setAlumnoForm] = useState({ nombre: '', dni: '', sexo: 'V', nuevoIngreso: false, editando: null });
  ...
  useEffect(() => {
    ...
    const estudiantesActualizados = alumnosDelGrado.map(alumno => {
      ...
      return {
        ...
        sexo: alumno.sexo || 'V', nuevoIngreso: !!alumno.nuevoIngreso,
        bimestres: {
  ...
  const agregarAlumno = async () => {
    ...
    if (alumnoForm.editando) {
      ...
      const nuevoIngresoVal = !!alumnoForm.nuevoIngreso;

      // 1. Actualizar en alumnosGlobales
      const idx = nuevos[gradoActual].findIndex(a => a.dni === dniOriginal);
      if (idx !== -1) nuevos[gradoActual][idx] = { nombre: nuevoNombre, dni: nuevoDni, sexo: nuevoSexo, nuevoIngreso: nuevoIngresoVal };
      ...
      await Promise.all(todasMaterias.map(async (m) => {
        ...
        nuevosEst[idxEst] = { ...nuevosEst[idxEst], nombre: nuevoNombre, dni: nuevoDni, sexo: nuevoSexo, nuevoIngreso: nuevoIngresoVal };
      ...
      const alumnoNuevo = { nombre: alumnoForm.nombre.trim(), dni: alumnoForm.dni.trim(), sexo: alumnoForm.sexo || 'V', nuevoIngreso: !!alumnoForm.nuevoIngreso };
      ...
      const estructuraVacia = {
        ...
        nuevoIngreso: alumnoNuevo.nuevoIngreso,
        bimestres: {
          1: { n1:'', n2:'', n3:'', n4:'', n5:'', nota:'', criteriosTexto:'', observacion:'' },
          2: { n1:'', n2:'', n3:'', n4:'', n5:'', nota:'', criteriosTexto:'', observacion:'' },
          3: { n1:'', n2:'', n3:'', n4:'', n5:'', nota:'', criteriosTexto:'', observacion:'' },
          ...
          <div style={{ padding: '24px 28px', maxWidth: '100%' }} className="fade-in">
            ...
            <div style={{ background: '#fff', border: '1.5px solid var(--border)', borderRadius: 'var(--r-lg)', padding: '20px 24px', marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                ...
                <label style={{ display: 'flex', gap: 8, alignItems: 'center', background: alumnoForm.nuevoIngreso ? '#fffbeb' : '#f8fafc', border: '1.5px solid', borderColor: alumnoForm.nuevoIngreso ? '#fde68a' : 'var(--border)', borderRadius: 'var(--r)', padding: '9px 14px', cursor: 'pointer' }}>
                  <input type="checkbox" checked={!!alumnoForm.nuevoIngreso}
                    onChange={e => setAlumnoForm({ ...alumnoForm, nuevoIngreso: e.target.checked })}
                    style={{ width: 16, height: 16, cursor: 'pointer', accentColor: '#d97706' }} />
                  <span style={{ fontSize: 13, fontWeight: 700, color: alumnoForm.nuevoIngreso ? '#92400e' : 'var(--muted)' }}>🆕 Nuevo ingreso</span>
                </label>
                ...
                {alumnoForm.editando && (
                  <button onClick={() => setAlumnoForm({ nombre: '', dni: '', sexo: 'V', nuevoIngreso: false, editando: null })}
              ...
              <p style={{ fontSize: 11, color: 'var(--muted)', marginTop: 8 }}>
                🆕 Marcá "Nuevo ingreso" para que este alumno se ubique siempre al final de la lista, sin importar el orden alfabético.
              </p>
            </div>
            ...
            <div style={{ background: '#fff', border: '1.5px solid var(--border)', borderRadius: 'var(--r-lg)', overflow: 'hidden', marginBottom: 20 }}>
              ...
              {alumnosGr.length === 0 ? (
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {[...alumnosGr].sort(compararAlumnos).map((a, i) => (
                      <tr key={a.dni} className="tabla-row" style={{ borderBottom: '1px solid #e2e8f0' }}>
                        <td style={{ padding: '11px 14px', textAlign: 'center', color: 'var(--muted)', fontSize: 14, fontWeight: 600 }}>{i + 1}</td>
                        <td style={{ padding: '11px 14px', fontWeight: 700, color: 'var(--text)', fontSize: 15 }}>
                          {a.nombre}
                          {a.nuevoIngreso && <span style={{ marginLeft: 8, background: '#fffbeb', color: '#92400e', border: '1px solid #fde68a', borderRadius: 20, fontSize: 10, fontWeight: 700, padding: '2px 8px' }}>🆕 Nuevo ingreso</span>}
                        </td>
                        ...
                        <td style={{ padding: '10px 14px', textAlign: 'center' }}>
                          <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
                            <button onClick={() => { setAlumnoForm({ nombre: a.nombre, dni: a.dni, sexo: a.sexo || 'V', nuevoIngreso: !!a.nuevoIngreso, editando: a }); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
